...
The MCP server publishes raw tool names without a product prefix: `get_context`, `list_projects`, `get_project`, `get_project_metrics`, `list_builds`, `get_project_logs`, `get_build_logs`, `list_deployments`, `get_project_health`, `list_healthcheck_failures`, `get_project_network`, `get_project_autoscaling`, `list_autoscaling_events`, `list_project_instances`, `get_valkey_status`, `list_ai_keys`, `get_ai_usage`, `get_project_billing`, and `list_transactions`. A client may prepend its configured server name, so server `zenifra` plus raw tool `get_context` can appear as `zenifra_get_context`. Never add the server namespace to a tool name that already contains it.

For MCP reads, start with `get_context`, then use `list_projects` or `get_project` to confirm the project type and exact authorized ID. Choose tools that match that type: application logs, health checks, network analytics and their failures apply to HTTP projects; `get_valkey_status` applies to Valkey projects. Use `list_project_instances` before passing an instance to metrics or logs, and use `list_builds` before requesting `get_build_logs` with a build ID. Continue build logs from `next_cursor`; stop when `finished` is true or no new chunks are returned. Use explicit time ranges and bounded pages. Empty lists, unavailable observations and null values must remain distinct from zero or success.
